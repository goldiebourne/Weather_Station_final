...
class DB:
    ...
    def getAllInRange(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weatherstation_db.find({self.datetime:start, }))
        except Exception as e:
            msg = str(e)
    ...
    def humidityMMAR(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weatherstation_db.aggregate( '''Add your Aggregation pipeline here in this function'''))
        except Exception as e:
            msg = str(e)
